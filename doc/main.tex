...
Tahapan pertama dalam \textit{compiler} ini adalah \textit{Lexical Analyzer} (Scanner) yang bertugas membaca \textit{source code} karakter demi karakter dari \texttt{SourceBuffer} dan mengelompokkannya menjadi \textit{Token} (seperti \textit{identifier}, \textit{keyword}, bilangan, dan operator). 

Implementasi pada \texttt{Scanner.cpp} dan \texttt{Scanner.hpp} dibangun dengan mereplikasi cara kerja \textit{Deterministic Finite Automaton} (DFA) yang dirangkum pada diagram transisi DFA. Alih-alih menggunakan matriks dua dimensi untuk mengatur status \textit{state}, implementasi DFA ini bersifat "\textit{Ad-Hoc}". \textit{State machine} dijalankan secara sekuensial melalui struktur kontrol (seperti \texttt{switch-case}, \texttt{while}, dan \texttt{if-else}).

Ketika perulangan \texttt{scanToken()} dipanggil, program mensimulasikan dimulainya pemrosesan dari \textit{Start State} ($q_0$). Bergantung pada karakter yang dibaca oleh \texttt{advance()}, cabang implementasi dijalankan sebagai berikut:
