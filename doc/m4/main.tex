...
Milestone 4 (M4) telah berhasil mengimplementasikan eksekusi kode perantara (P-Code) untuk bahasa Pascal versi subset. Implementasi meliputi interpreter virtual stack machine yang mendukung instruksi dasar (LIT, LOD, STO, INT, JMP, JPC, CAL, RET), operasi aritmatika (ADD, SUB, MUL, DIV, MOD), perbandingan (EQL, NEQ, GTR, LSS, GEQ, LEQ), I/O (OPR 14), serta fitur lanjutan seperti prosedur dengan parameter, function dengan return value, array dengan bounds checking, dan statement \texttt{case}.

Pengujian integrasi pada 14 kasus uji menunjukkan bahwa implementasi dapat menangani berbagai skenario program, mulai dari operasi dasar, kontrol alur (\texttt{if}, \texttt{while}, \texttt{for}, \texttt{repeat-until}), hingga deteksi kesalahan runtime (overflow, division by zero, bounds checking). Kode perantara yang dihasilkan konsisten dengan ekspetasi P-Code, menunjukkan bahwa compiler (M1--M3) dan interpreter (M4) bekerja secara terintegrasi dengan baik.
