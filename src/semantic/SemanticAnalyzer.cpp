...
SemanticAnalyzer::SemanticAnalyzer(SymbolTableManager& symTable) : symTable(symTable) {}
...
void SemanticAnalyzer::printErrors() const {
    for (const auto& err : errors) {
        std::cerr << "Semantic error on line " << err.line << ": " << err.message << std::endl;
    }
}
...
void SemanticAnalyzer::checkConditionType(std::shared_ptr<ASTNode> condition, const std::string& context) {
    if (!condition) return;
    if (condition->type != TYPE_BOOLEAN) {
        reportError(condition->line, context + " condition must be Boolean, got " + codeToTypeName(condition->type));
    }
}
...
int SemanticAnalyzer::resolveNodeType(std::shared_ptr<ASTNode> typeNode) {
    if (!typeNode) return TYPE_UNKNOWN;
...
bool SemanticAnalyzer::checkAssignmentCompatibility(int targetType, int valueType) {
    return TypeSystem::isAssignmentCompatible(targetType, valueType);
}

int SemanticAnalyzer::inferBinaryOpType(const std::string& op, int leftType, int rightType) {
    ...
    if (op == "==" || op == "<>" || op == "<" || op == "<=" || op == ">" || op == ">=") {
        if (!isCompatible(leftType, rightType) && !(isNumeric(leftType) && isNumeric(rightType))) {
            return TYPE_UNKNOWN;
        }
        return TYPE_BOOLEAN;
    }
...
int SemanticAnalyzer::inferUnaryOpType(const std::string& op, int operandType) {
    if (op == "not") {
        if (operandType != TYPE_BOOLEAN) return TYPE_UNKNOWN;
        return TYPE_BOOLEAN;
    }
    if (op == "+" || op == "-") {
        if (!isNumeric(operandType)) return TYPE_UNKNOWN;
        return operandType;
    }
    return TYPE_UNKNOWN;
}
...
static int detectLiteralType(const std::string& val) {
    if (val == "true" || val == "false") return TYPE_BOOLEAN;
    if (!val.empty() && val.front() == '\'' && val.back() == '\'') {
        if (val.length() == 3) return TYPE_CHAR;
        return TYPE_STRING;
    }
    ...
    for (size_t i = start; i < val.size(); ++i) {
        char c = val[i];
        if (c == '.') hasDot = true;
        else if (c == 'e' || c == 'E') hasExp = true;
        else if (!isdigit(c)) {
            // Not a pure literal - could be an identifier constant reference
            return TYPE_UNKNOWN;
        }
    }
    if (hasDot || hasExp) return TYPE_REAL;
    return TYPE_INTEGER;
}
...
void SemanticAnalyzer::visit(VarDeclNode& node) {
    ...
    if (node.typeNode) {
        ...
        if (auto arr = std::dynamic_pointer_cast<ArrayTypeNode>(node.typeNode)) {
            typeCode = TYPE_ARRAY;
            ...
            if (auto r = std::dynamic_pointer_cast<RangeNode>(arr->indexType)) {
                if (r->low && r->high) {
                    try {
                        low = std::stoi(std::dynamic_pointer_cast<LiteralNode>(r->low)->value);
                        high = std::stoi(std::dynamic_pointer_cast<LiteralNode>(r->high)->value);
                    } catch (...) {}
                }
                if (low > high) {
                    reportError(node.line, "Array lower bound exceeds upper bound");
                }
            }
            int elsz = 1;
            int size = (high - low + 1) * elsz;
            ref = symTable.enterArray(xtyp, etyp, 0, low, high, elsz, size);
        } else if (auto rec = std::dynamic_pointer_cast<RecordTypeNode>(node.typeNode)) {
            ...
            ref = recBlock;
            symTable.exitBlock();
        } else if (auto r = std::dynamic_pointer_cast<RangeNode>(node.typeNode)) {
...
void SemanticAnalyzer::visit(FuncDeclNode& node) {
    ...
    if (retType == TYPE_UNKNOWN) {
        reportError(node.line, "Unknown return type '" + node.returnTypeName + "'");
    }
...
void SemanticAnalyzer::visit(AssignNode& node) {
    ...
    if (!checkAssignmentCompatibility(targetType, valueType)) {
        reportError(node.line,
            "Type mismatch in assignment: expected " + codeToTypeName(targetType) +
            ", got " + codeToTypeName(valueType));
    }
}
...
void SemanticAnalyzer::visit(ForNode& node) {
    ...
    if (varIdx == -1) {
        reportError(node.line, "Undeclared loop variable '" + node.varName + "'");
    } else {
    ...
    if (node.initExpr) {
        ...
        if (varType != TYPE_UNKNOWN && !checkAssignmentCompatibility(varType, node.initExpr->type)) {
            reportError(node.line,
                "For loop initial value incompatible with loop variable: expected " +
                codeToTypeName(varType) + ", got " + codeToTypeName(node.initExpr->type));
        }
    }

    if (node.finalExpr) {
        node.finalExpr->accept(*this);
        if (varType != TYPE_UNKNOWN && !checkAssignmentCompatibility(varType, node.finalExpr->type)) {
            reportError(node.line,
                "For loop final value incompatible with loop variable: expected " +
                codeToTypeName(varType) + ", got " + codeToTypeName(node.finalExpr->type));
        }
    }
            ...
            if (auto cb = std::dynamic_pointer_cast<CaseBranchNode>(branch)) {
                for (auto& c : cb->constants) {
                    ...
                    if (c && c->type != TYPE_UNKNOWN && c->type != exprType) {
                        reportError(c->line,
                            "Case constant type mismatch: expected " + codeToTypeName(exprType) +
                            ", got " + codeToTypeName(c->type));
                    }
                }
            }
            branch->accept(*this);
...
void SemanticAnalyzer::checkProcedureArgs(int procIdx,
    const std::vector<std::shared_ptr<ASTNode>>& args, int line) {
    ...
    if (blockIdx < 0 || blockIdx >= symTable.btabSize()) return;
    ...
    if (args.size() != paramIndices.size()) {
        reportError(line,
            "Wrong number of arguments: expected " + std::to_string(paramIndices.size()) +
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        if (!checkAssignmentCompatibility(expectedType, args[i]->type)) {
            reportError(args[i]->line,
                "Argument " + std::to_string(i + 1) + " type mismatch: expected " +
                codeToTypeName(expectedType) + ", got " + codeToTypeName(args[i]->type));
        }
    }
}
...
void SemanticAnalyzer::visit(ProcCallNode& node) {
    ...
    if (entry.obj != OBJ_PROCEDURE && entry.obj != OBJ_FUNCTION) {
        reportError(node.line, "'" + node.name + "' is not a callable procedure or function");
    }
...
void SemanticAnalyzer::visit(BinaryOpNode& node) {
    ...
    if (node.op == "+" || node.op == "-" || node.op == "*" ||
        ...
        if (!isNumeric(leftType) || !isNumeric(rightType)) {
            reportError(node.line,
                "Arithmetic operator '" + node.op + "' requires numeric operands, got " +
                codeToTypeName(leftType) + " and " + codeToTypeName(rightType));
        }
        node.type = (leftType == TYPE_REAL || rightType == TYPE_REAL) ? TYPE_REAL : TYPE_INTEGER;
    }
    else if (node.op == "==" || node.op == "<>" || node.op == "<" ||
             node.op == "<=" || node.op == ">" || node.op == ">=") {
        if (!isCompatible(leftType, rightType) && !(isNumeric(leftType) && isNumeric(rightType))) {
            reportError(node.line,
                "Incompatible types in comparison: " + codeToTypeName(leftType) +
                " and " + codeToTypeName(rightType));
        }
        ...
        if (leftType != TYPE_BOOLEAN || rightType != TYPE_BOOLEAN) {
            reportError(node.line,
                "Logical operator '" + node.op + "' requires Boolean operands, got " +
                codeToTypeName(leftType) + " and " + codeToTypeName(rightType));
        }
        node.type = TYPE_BOOLEAN;
    }
...
void SemanticAnalyzer::visit(UnaryOpNode& node) {
    ...
    if (node.op == "not") {
        if (operandType != TYPE_BOOLEAN) {
            reportError(node.line,
                "'not' requires Boolean operand, got " + codeToTypeName(operandType));
        }
        node.type = TYPE_BOOLEAN;
    }
        ...
        if (!isNumeric(operandType)) {
            reportError(node.line,
                "Unary '" + node.op + "' requires numeric operand, got " + codeToTypeName(operandType));
        }
...
void SemanticAnalyzer::visit(ArrayAccessNode& node) {
    ...
    if (arrType != TYPE_ARRAY) {
        reportError(node.line, "Array access on non-array type " + codeToTypeName(arrType));
        node.type = TYPE_UNKNOWN;
        return;
    }
    ...
    if (arrTabIdx != -1) {
        ...
        if (ref >= 0 && ref < symTable.atabSize()) {
            ...
            for (auto& idx : node.indices) {
                if (idx) {
                    idx->accept(*this);
                    if (!isAssignmentCompatible(ate.xtyp, idx->type) && idx->type != TYPE_INTEGER && idx->type != TYPE_SUBRANGE) {
                        reportError(idx->line,
                            "Array index type mismatch: expected " + codeToTypeName(ate.xtyp) +
                            ", got " + codeToTypeName(idx->type));
                    }
                }
            }
            return;
...
void SemanticAnalyzer::visit(FieldAccessNode& node) {
    ...
    if (recType != TYPE_RECORD) {
        reportError(node.line, "Field access on non-record type " + codeToTypeName(recType));
        node.type = TYPE_UNKNOWN;
        return;
    }
...
void SemanticAnalyzer::visit(RangeNode& node) {
    ...
    if (lowType != highType && !(isNumeric(lowType) && isNumeric(highType))) {
        reportError(node.line,
            "Range bounds must have same type, got " + codeToTypeName(lowType) +
            " and " + codeToTypeName(highType));
    }
    ...
    if (auto litLow = std::dynamic_pointer_cast<LiteralNode>(node.low)) {
        if (auto litHigh = std::dynamic_pointer_cast<LiteralNode>(node.high)) {
            try {
                ...
                if (l > h) {
                    reportError(node.line,
                        "Range lower bound (" + litLow->value + ") exceeds upper bound (" +
                        litHigh->value + ")");
                }
            } catch (...) {}
        }
